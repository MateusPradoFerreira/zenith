...
@Component({
  ...
  template: `
    <hlm-trow *ngIf="data().type === 'BANK'? data()?.children?.length : true" (click)="handleClickRow()" class="{{ level() >= 2? 'hover:bg-slate-50' : data().type === 'MARK' || data().type === 'BANK'? ( level()? 'bg-slate-50' : 'bg-slate-100 hover:bg-slate-100') : 'hover:bg-white' }} {{ level()? 'cursor-pointer' : '' }}">
      <hlm-td class="flex-1 {{ data().type === 'PERIOD' ? 'bg-slate-50' : '' }}" truncateClass="{{ level() === 1? 'flex' : '' }} gap-4 justify-between items-center">
        <span>{{ data().name }}</span>
        <i-lucide *ngIf="level() === 1" [name]="opened()? 'chevron-down' : 'chevron-up'" size="16" class="cursor-pointer" />
      </hlm-td>
      <hlm-td class="w-31 justify-end hidden {{ i > 5? 'flex' : i > 3? '1xl:flex' : '2xl:flex' }} {{ level()? 'text-slate-500' : data().type === 'RECEIVABLE_MARK'? 'font-medium text-emerald-500' : data().type === 'PAYABLE_MARK'? 'font-medium text-red-500' : data().type === 'PERIOD'? 'font-medium bg-slate-50 text-muted-foreground' : 'font-medium text-blue-500' }}" *ngFor="let value of data().values; index as i">
        <span *ngIf="data().type !== 'PERCENT' && data().type !== 'PERIOD'">{{ value | currency: "BRL" }}</span>
        <span *ngIf="data().type === 'PERIOD'">{{ value | date : 'dd/MM/YYYY' }}</span>
