...
@Component({
  ...
  template: `
    <hlm-trow (click)="handleToggleOpend()" class="{{ level() >= 2? 'hover:bg-slate-50' : data().type === 'MARK'? ( level()? 'bg-slate-50' : 'bg-slate-100 hover:bg-slate-100') : 'hover:bg-white' }} {{ level()? 'cursor-pointer' : '' }}">
      ...
      <hlm-td class="truncate w-29 justify-end {{ level()? 'text-slate-500' : data().type === 'RECEIVABLE'? 'font-medium text-emerald-500' : data().type === 'PAYABLE'? 'font-medium text-red-500' : 'font-medium text-blue-500' }}" *ngFor="let value of data().values">{{ value | currency: "BRL" }}</hlm-td>
    </hlm-trow>
