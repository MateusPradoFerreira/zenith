...
export const inputVariants = cva(
	'flex rounded-md border w-full font-normal border-slate-200 bg-transparent text-base md:text-sm ring-offset-background read-only:bg-slate-50 read-only:text-slate-700 file:border-0 file:text-foreground file:bg-transparent file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-none focus-visible:ring-ring focus-visible:ring-offset-2 :cursor-not-allowed disabled:bg-sldisabledate-50 disabled:text-slate-700 peer',
	{
		variants: {
			size: {
				default: 'py-2 px-4 file:max-md:py-0',
...
@Directive({
	...
	providers: [
		{ provide: BrnFormFieldControl, useExisting: HlmInputDirective },
	],
})
export class HlmInputDirective implements BrnFormFieldControl, DoCheck, AfterViewInit {
    ...
    if (!input.placeholder) this.renderer.setAttribute(input, "placeholder", "");
	}
