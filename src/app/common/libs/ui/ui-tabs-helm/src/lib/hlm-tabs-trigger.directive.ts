...
export class HlmTabsTriggerDirective {
	...
	protected _computedClass = computed(() =>
		hlm(
			'inline-flex cursor-pointer duration-300 border-b-2 border-transparent items-center justify-center whitespace-nowrap rounded-t-sm px-4 h-full pt-0.5 text-xs font-medium transition-all focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 data-[state=active]:text-violet-500 data-[state=active]:border-violet-300 mb-[-1px]',
			this.userClass(),
		),
	);
}
