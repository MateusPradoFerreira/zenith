		...
		<div class="flex items-center justify-between gap-2 px-4 py-2">
			...
			<nav hlmPagination>
				<ul hlmPaginationContent>
					@if (showEdges()) {
						<li hlmPaginationItem (click)="goToPrevious()" [class]="isFirstPageActive()? 'pointer-events-none' : ''">
							<hlm-pagination-previous [disabled]="isFirstPageActive()"/>
						</li>
					}
					...
					@if (showEdges()) {
						<li hlmPaginationItem (click)="goToNext()" [class]="isLastPageActive()? 'pointer-events-none' : ''">
							<hlm-pagination-next [disabled]="isLastPageActive()"/>
						</li>
					}
				</ul>
			</nav>
			...
			<brn-select [(ngModel)]="itemsPerPage" class="ml-auto" placeholder="Page size">
				...
				<hlm-select-content>
					@for (pageSize of pageSizesWithCurrent(); track pageSize) {
						<hlm-option [value]="pageSize">{{ pageSize }}/pg</hlm-option>
					}
				</hlm-select-content>
			</brn-select>
		</div>
...
export class HlmNumberedPaginationComponent {
	 ...
	 */
	public readonly pageSizes = input<number[]>([10, 25, 50, 75, 100]);
